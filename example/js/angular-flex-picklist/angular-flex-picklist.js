...
(function (root, factory) {
    ...
    var flexPicklist = function (Pagination) {
        return {
            restrict: 'E',
            scope: {
                searchPlaceHolder: '@',
            ...
            templateUrl: currentScriptPath.replace('angular-flex-picklist.js', 'angular-flex-picklist.html'),
            link: function (scope, element, attrs) {
				...
				scope.updateFilteredItems = function () {
                    if (scope.search.optionsRightFilter !== "") {
                        var filtered = scope.target.filter(scope.filterRightItems);
                    ...
                    if (scope.search.optionsLeftFilter !== "") {
                        var filtered = scope.source.filter(scope.filterLeftItems);
                ...
                scope.moveItemRight = function () {
                    ...
                    var index = scope.source.indexOf(scope.selectedLeftItem);
				...
				scope.moveAllItemsRight = function () {
					for(var index = 0 in scope.source){
						scope.target.push(scope.source[index]);
					}
                ...
                scope.moveItemLeft = function () {
                    ...
                    var index = scope.target.indexOf(scope.selectedRightItem);
				...
				scope.moveAllItemsLeft = function () {
					for(var index = 0 in scope.target){
						scope.source.push(scope.target[index]);
					}
    ...
    angular.module('angularflexPicklist', ["simplePagination"])
        .directive('flexPicklist', flexPicklist);
